...
THINKING_LEVELS=("disabled" "high" "max")
...
for model in "${MODELS[@]}"; do
  ...
  for thinking in "${THINKING_LEVELS[@]}"; do
    ...
    if [ "$thinking" == "max" ]; then
        # Default is max effort, we can just omit kwargs or pass explicit max
        model_kwargs='{"extra_body": {"chat_template_kwargs": {"reasoning_effort": "max"}}}'
    elif [ "$thinking" == "high" ]; then
        model_kwargs='{"extra_body": {"chat_template_kwargs": {"reasoning_effort": "high"}}}'
